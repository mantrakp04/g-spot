...
interface PiAgentDefaultsCardInnerProps {
...
function PiAgentDefaultsCardInner({
  allModels,
  configuredProviders,
  oauthProviders,
  tools,
...
/**
 * Self-contained global Pi agent defaults card — fetches its own catalog and
 * defaults via tRPC. Used on `/chat/settings` and in the onboarding agent step.
 */
export function PiAgentDefaultsCard() {
  ...
  if (piCatalog.isLoading || piDefaults.isLoading) {
    return <Skeleton className="h-[720px] w-full rounded-xl" />;
  }

  return (
    <PiAgentDefaultsCardInner
      allModels={allModels}
      configuredProviders={configuredProviders}
      oauthProviders={oauthProviders}
      tools={tools}
